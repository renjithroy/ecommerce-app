...
<?php if($option == "allTimePayments") {

    $stmt1 = $conn->prepare("SELECT * FROM payments"); //returns total no.of products in database

    $stmt1->execute();

    $allTimePayments = $stmt1->get_result();

}else if($option == "sixMonthPayments") {

    $stmt1 = $conn->prepare("SELECT * FROM payments WHERE payment_date >= date_sub(now(),interval 6 month) "); //returns total no.of products in database

    $stmt1->execute();

    $sixMonthPayments = $stmt1->get_result();

}else if($option == "previousMonthPayments") {

    $stmt2 = $conn->prepare("SELECT * FROM payments WHERE YEAR(payment_date) = YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AND MONTH(payment_date) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH)"); //returns total no.of products in database

    $stmt2->execute();

    $previousMonthPayments = $stmt2->get_result();
  ...
  <div class="nav-wrap">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 ">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom" style="margin-bottom: 20px !important;">
        <h1 class="h2" style="padding-left: 28rem">Payments</h1>
      ...
      <div class="table-responsive" style="margin-top: 23px !important">
        <table class="table table-striped table-sm text-center sortable">
          ...
          <tbody>
          <?php if($option == "allTimePayments") { ?>
            <?php foreach($allTimePayments as $payment) { ?>
                <tr>
                  <td class="text-center"><?php echo $payment['payment_id'] ?></td>
                  <td class="text-center"><?php echo $payment['order_id'] ?></td>
                  <td class="text-center"><?php echo $payment['user_id'] ?></td>
                  <td class="text-center"><?php echo $payment['transaction_id'] ?></td>
                  <td class="text-center"><?php echo $payment['payment_date'] ?></td>
                </tr>
            <?php } ?>
            <?php }else if($option == "sixMonthPayments") { ?>
            <?php foreach($sixMonthPayments as $payment) { ?>
                <tr>
                  <td class="text-center"><?php echo $payment['payment_id'] ?></td>
                  <td class="text-center"><?php echo $payment['order_id'] ?></td>
                  <td class="text-center"><?php echo $payment['user_id'] ?></td>
            ...
            <?php } ?>
          <?php }else if($option == "previousMonthPayments") { ?>
            <?php foreach($previousMonthPayments as $payment) { ?>
                <tr>
                  <td class="text-center"><?php echo $payment['payment_id'] ?></td>
                  <td class="text-center"><?php echo $payment['order_id'] ?></td>
                  <td class="text-center"><?php echo $payment['user_id'] ?></td>
